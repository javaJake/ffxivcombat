...
public class Functions {
  ...
  public int fWD() {
    return (int)
        // f(WD) = ⌊ ( LevelMod[Lv, MAIN] · JobMod[Job, Attribute] / 1000 ) + WD ⌋
   ...
   *
   * @return total Determination’s contribution to damage and healing output as a rational number (rather than percentage)
   */
  public int fDET() {
    return (int)
  ...
  public int fTEN() {
    switch (jobMod) {
      case PLD, WAR, DRK, GNB -> {
        return (int)
            // f(TNC) = ⌊ 100 · ( TNC - LevelMod[Lv, SUB] )/ LevelMod[Lv, DIV] + 1000 ⌋
  ...
  public int fCRIT() {
    return (int)
        // f(CRIT) = ⌊ 200 · ( CRIT - LevelMod[Lv, SUB] )/ LevelMod[Lv, DIV] + 1400 ⌋
